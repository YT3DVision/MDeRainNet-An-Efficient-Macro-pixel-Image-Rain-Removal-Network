...
pre = np.zeros((1, 3, 320, 320))
pre = torch.Tensor(pre)
pre = Variable(pre).to('cuda:0')
input = np.zeros((1, 3, 320, 320))
input = torch.Tensor(input)
input = Variable(input).to('cuda:0')
label = np.zeros((1, 3, 320, 320))
label = torch.Tensor(label)
label = Variable(label).to('cuda:0')
CR = ContrastLoss(False)

loss = CR(pre, label, input)
print(loss)
